@echo off
REM ============================================================================
REM  env_reset.bat -- clear EVERY REP_* variable the engines read, plus RESULT and the
REM  names that were REMOVED (REP_PRELOAD, REP_INFO, KNA2_RESULT, REP_F3CAP, REP_F3L4,
REM  REP_CANDCAP).  Those now STOP a run, so a leftover one in the shell would abort a case
REM  rather than quietly change it -- either way the case must not see it.  Called (CALL, so
...
for %%V in (
  ...
  REP_F2MAX REP_F3A REP_F3CAP REP_F3COMPLETE REP_F3COUNT REP_F3DUMP REP_F3L4 REP_F3LIST
  REP_F3MAX REP_F3RAW REP_F3START REP_F3STOP REP_FANOUT REP_FPF REP_FTEST REP_INFO REP_LEVEL REP_LEVELSTATS
  ...
  REP_PATTERN REP_PRELOAD REP_PRINTEACH REP_PRINTRAW REP_PRIORITY REP_PRUNELEVEL REP_RANGE
  REP_RAWSHARD REP_REPSCHECK REP_ROW4 REP_SAMPLE REP_SGEN REP_SWAPCAP REP_SYMCAP REP_TRIPLES REP_TYPEMASK
