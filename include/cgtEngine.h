...
template<int N> inline std::vector<Perm<N>> setwiseStab(const BSGS<N>& BG,
    ...
    // membership test set, as a reused sorted vector (binary_search) -- same result as a
    // std::set<Perm> but no per-node red-black-tree node allocations (this routine runs once
    // per over-cap search node, so its temporaries dominated the profile: cgtEngine alloc storm).
    thread_local std::vector<Perm<N>> fsort;
    fsort.assign(factors.begin(), factors.end());
    ...
    auto predFull = [&](const Perm<N>& g) -> bool {          // g permutes the factor set
        for (auto& F : factors) if (!std::binary_search(fsort.begin(), fsort.end(), applyPerm<N>(F, g.data()))) return false;
        return true;
    };
    ...
    auto buildChain = [&](const Perm<N>& pre, int l, std::vector<Perm<N>>& sk) {
        // reused per-thread scratch: ns (next-level gens, sorted-unique vector == the old
        // std::set) and orbitTransversal buffers. No per-call malloc.
        thread_local std::vector<Perm<N>> ns;
        thread_local std::vector<int> orb;
        thread_local std::array<Perm<N>, N> rep; thread_local std::array<char, N> vis;
        sk.assign(Kgens.begin(), Kgens.end());
        for (int j = 0; j < l && !sk.empty(); j++) {
            int fp = pre[BG.base[j]];
            orbitTransversal<N>(fp, sk, rep, vis, orb);
            ns.clear();
            for (int x : orb) for (auto& g : sk) { Perm<N> s = composePerm<N>(invPerm<N>(rep[g[x]]), composePerm<N>(g, rep[x])); bool id = true; for (int t = 0; t < N; t++) if (s[t] != t) { id = false; break; } if (!id) ns.push_back(s); }
            std::sort(ns.begin(), ns.end()); ns.erase(std::unique(ns.begin(), ns.end()), ns.end());
            sk.assign(ns.begin(), ns.end());
        }
    };
    auto kMinimal = [&](const std::vector<Perm<N>>& sk, int cl) -> bool {  // cl minimal in its <sk>-orbit?
        if (sk.empty()) return true;                         // trivial chain -> everything minimal
        thread_local std::vector<int> orb;
        thread_local std::array<Perm<N>, N> rep; thread_local std::array<char, N> vis;
        orbitTransversal<N>(cl, sk, rep, vis, orb);
        for (int x : orb) if (x < cl) return false;
        return true;
    };
    ...
    std::function<void(int, Perm<N>)> rec = [&](int l, Perm<N> pre) {
        ...
        int orb[N]; int no = (int)BG.orbit[l].size();       // stack copy (orbit <= N points); no heap
        for (int t = 0; t < no; t++) orb[t] = BG.orbit[l][t];
        std::sort(orb, orb + no, [&](int a, int b) { return pre[a] < pre[b]; });   // ascending image order
        auto& sk = chainPool[l];
        size_t kAt = (size_t)-1;                            // Kgens size the chain was built at (-1 = stale)
        for (int oi = 0; oi < no; oi++) {
            int o = orb[oi];
            Perm<N> cur = composePerm<N>(pre, BG.urep[l][o]);
            if (!partialOK(cur, l)) continue;
            if (kAt != Kgens.size()) { buildChain(pre, l, sk); kAt = Kgens.size(); }
            if (!kMinimal(sk, cur[BG.base[l]])) continue;
            rec(l + 1, cur);
        }
    };
    rec(0, idPerm<N>());
