...
#pragma pop_macro("new")
    ...
    K16A2(const FactorParams& factParam, int fixed3RowsIndex, int kThreads, const unsigned char* first3Rows, ResultCallback callback, void* cbClassPtr = NULL, bool bPrint = true,
          const unsigned char* v4Row = NULL, const unsigned char* v4Val = NULL);
    ~K16A2() {}
...
private:
    ...
    // pairing 0-with-m_v4Row[i] must pair 1-with-m_v4Val[i]. 0 = filter unused.
    uint8_t m_v4Row[4] = { 0, 0, 0, 0 };
    uint8_t m_v4Val[4] = { 0, 0, 0, 0 };
    ...
    void finishRepTable();                                    // the log table's = TOTAL row; called once, after the last REP_ORDERS token
