@echo off
REM ============================================================================
REM  runs\K18-t8-a0 -- K18 |Aut| = 2, TYPE 8 (2^8.1^2), a = 0 stratum.
REM  A full sweep gives 9,626 classes, aut{2:9447 4:144 8:22 16:12 272:1} -- every
REM  order-4, order-8, order-16 and the order-272 class in the K18 catalog is
REM  t8-type.  AllResults\K18-t8-a0.txt holds that answer -- a re-run must
REM  REPRODUCE it, not read it.
REM
REM  RUN THIS .BAT AS IS for the full sweep: RAW blocks c = 0 .. 57751, of which
REM  25,530 are canonical and actually completed (the last is 0.0.48767).
REM
REM  Both range variables are ALWAYS set -- there is no "run to the end" default:
REM      REP_F3START   first RAW block c                 (default 0)
REM      REP_F3STOP    last RAW block c, INCLUSIVE      (default 57751 = the last one)
...
REM
REM  ACCELERATORS: sigma0 = 2^8.1^2 has two FIXED POINTS, and that lost symmetry
REM  rejects REP_TYPEMASK / REP_SGEN / REP_PATAPPLY; REP_DIAGPRUNE needs REP_F3A=4.
...
REM  blocks 0-9, 10 threads: 530.1s -> 102.6s, a 5.2x speedup, with the SAME TREE
REM  (243,116,032 nodes against 243,112,960) and the same 22 classes, compare.pl
...
REM  never write over each other:  K18-t8-a0_<start>-<stop>.log and K18-t8-a0_<start>-<stop>.txt
