# K18-t9-a0 -- what this case computes

One-factorizations of K18 whose full symmetry group has order 2, whose symmetry is
fixed-point-free, and which fall in branch a = 0. This is the census leg that produces the
727 classes.
...
The log is one table, one row per block:
    ...
    Factorization of K18 P1F |Aut| = 2, symmetry type 2⁹, branch a=0
    Blocks 0-57215 selected for processing (REP_F3START=0 REP_F3STOP=57215)
...
For a = 0 the fixed round is exactly those 9 pairs and nothing else.

Only two branches exist. The dichotomy theorem shows d is either 9 or 1, so a is 0 or 4
and nothing else -- a = 1, 2 and 3 are impossible by proof, not merely unfound. The a = 4
branch is the sibling case K18-t9-a4.

## Blocks, and who gets credit for a solution

The column is 57,216 blocks, or starters: the distinct ways of filling in the opening
...
Of the 57,216 raw blocks, 8,713 are canonical and actually completed, the last being
0.0.47925. The rest are relabelings of an earlier block, are recognized as such quickly,
...
2. **Its symmetry group is bigger than order 2.** A design whose full group has order 3,
   4, 272 and so on may contain a symmetry of order 2, so this sweep can find it -- but it
   is defined and published by the K18Aut3-All case, not by this one. Writing it here
   would duplicate that case's output.
...
block that owns it. This branch is expected to contribute nothing to it: every K18 class
with a group larger than order 2 that has a fixed-point-free order-2 symmetry sits in the
a = 4 branch.

## No accelerator flags

`REP_TYPEMASK`, `REP_SGEN` and `REP_PATAPPLY=oneperpair` are unsound for a = 0. The engine
accepts them and silently returns nothing, so they are not set here. `REP_DIAGPRUNE` is
refused outright unless the branch is a = 4. `REP_PRUNELEVEL=1` is the only speed setting
this case uses.
...
The 727 classes are all in the catalog, `AllResults/K18_P1F_aut_gt1.txt`, and the Compare step
below checks a run against it. The raw output of the sweep that produced them is not kept any
more; it lives in git history.
