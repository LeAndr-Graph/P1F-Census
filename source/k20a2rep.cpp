...
void K20A2::runRepresentativeMethod(int order, int target) {
    ...
    if (g_precalc && m_bPrint) printf("[K20-REP] REP_PRECALC: PROTOTYPE -- precalculated orbit lists below each seed; trivial-stabilizer nodes only (use REP_PRUNELEVEL=1)\n");
    ...
    for (int typeIdx = 0; typeIdx < g_numTypes; typeIdx++) {
        ...
        auto setupTick = [&](const char* stage, long long n) {
            ...
            setupT0 = now;
            printf("[K20-REP]  type %-14s SETUP %-14s %8.1fs  n=%lld\n", tstr, stage, s, n);
            fflush(stdout);
        };
        ...
        if (sh.Calpha.empty() && diveMode && shardLevel == 0) {   // pure fast search (no sweep); with REP_LEVEL the sweep's runBlock dives per block
            ...
            if (rangeMode && chunkStep > 0) {
                ...
                if (m_bPrint) { printf("[K20-REP]  type %-14s SHARD level %d: built range [%zu, %zu) = %zu branches (%.1fs) -> run (work-queue, all threads)...\n", tstr, shardLevel, rangeStart, rangeEnd, shardFront.size(), elapsed()); fflush(stdout); }
            } else {
                ...
                if (m_bPrint) { printf("[K20-REP]  type %-14s LEVEL 1 branches %zu (%.1fs)\n", tstr, shardFront.size(), elapsed()); fflush(stdout); }
                for (int lvl = 2; lvl <= shardLevel && !shardFront.empty(); lvl++) {
                    if (lvl == shardLevel) {
                        ...
                        for (auto& th : bp) th.join();
                        if (m_bPrint) { printf("[K20-REP]  type %-14s LEVEL %d branches %lld (%.1fs, counted -- not stored)\n", tstr, lvl, cnt.load(), elapsed()); fflush(stdout); }
                        break;
                    }
                    ...
                    if (m_bPrint) { printf("[K20-REP]  type %-14s LEVEL %d branches %zu (%.1fs%s)\n", tstr, lvl, shardFront.size(), elapsed(), par ? ", parallel" : ""); fflush(stdout); }
                }
            ...
            if (shardLevel > 0 && m_bPrint) {
                printf("[K20-REP]  type %-14s LEVEL 1 branches %zu (root reps -- the partition on the enumerable path; the SETUP lines above are where the time went)\n", tstr, reps.size());
                fflush(stdout);
            }
            if (deepShard) {
                auto tS = std::chrono::steady_clock::now();
                ...
                if (hi < lo) hi = lo;
                if (m_bPrint) { printf("[K20-REP]  type %-14s SHARD range [%zu, %zu) of %zu root reps -> run...\n", tstr, lo, hi, reps.size()); fflush(stdout); }
            ...
            if (rangeMode && deepShard && shardHi > 0) {
                        ...
                        for (;;) {
                            ...
                            if (g_precalc) {
                                if (!orb.root) {             // a seed: build its list once; every entry is admissible at the seed
                                    ...
                                    if (m_bPrint) { std::lock_guard<std::mutex> lk(g_print_mtx); printf("[K20-REP]  type %-14s PRECALC: seed list %zu orbits (%.1fs)\n", tstr, orb.root->ent.size(), std::chrono::duration<double>(std::chrono::steady_clock::now() - tL).count()); fflush(stdout); }
                                }
                ...
                for (size_t bi = lo; bi < hi && !g_stop.load(std::memory_order_relaxed); bi++) {
                    ...
                    if (m_bPrint) { printf("[K20-REP]  type %-14s BLOCK %zu: %zu branches -> %d threads, next branch in sequence\n", tstr, bi, front.size(), nThreads); fflush(stdout); }
