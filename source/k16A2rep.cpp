...
// NOTE k16 HAS NO REP_PRUNELEVEL (see the note in k20a2rep.cpp where it is read: k16 was left
// without it deliberately).  In general that means the generator still runs its per-node
// setwiseStab dedup where this path explores every orbit, so the two trees need not agree.
// MEASURED on the K16 order-3 oracle: they agree
// exactly -- 1,349,431 nodes each side, same 20 classes {3:19 15:1}, result files identical
// after sorting -- because the block-root stabilizer on these types is already trivial, so the
// generator's setwiseStab returns {} anyway.  The class set is what must match; on this case the
// node count matches too, and the run went 233 -> 726 nodes/ms.
//
// The k18 t9 a=4 thread-scaling test located its loss in this walk: two 1-thread processes
// scaled 1.67x and the plain path 1.62x, but one process with two threads only 1.35x. The per-node make_shared, the per-child copies of the partial cover and
...
std::atomic<long long> g_blkN1{ 0 }, g_blkN2{ 0 }, g_blkN5{ 0 }, g_blkN10{ 0 };  // per-block: covers that are EXACTLY the 1st/2nd/5th/10th detection of their class (occurrence-ordinal buckets; n1 = newDistinct)
FILE* g_estFile = nullptr;            // REP_ESTFILE: append "<blockIdx> <covers> <newDistinct>" per FULL block; null = off
std::atomic<long long> g_curBlock{ -1 };  // REP_EVENTFILE: block index of the block being scanned right now (set per sweep block; -1 outside the sweep)
FILE* g_eventFile = nullptr;          // REP_EVENTFILE: one line "<classIdx> <blockIdx>" per COVER (every detection incl. re-finds); null = off. The FULL event list -- every ordinal/occurrence statistic derives from it.
struct ClassInfo {   // per-class: index + 2*|Aut| + THREE candidate canonical-representative rules and the block each landed in.
    int idx; int aut2;
    std::string minRaw;  long long canonBlk;       // lex-MIN serialization  (keyed on FRONT bytes => correlated with block prefix => expected FRONT-loaded)
    std::string minRev;  long long canonBlkRev;    // reverse-lex min        (keyed on BACK bytes  => de-correlated from prefix => expected UNIFORM)
    size_t       minHash; long long canonBlkHash;  // min hash(serialization)(fully uncorrelated    => UNIFORM control)
};
std::map<std::string, ClassInfo> g_classInfo;  // canonKey -> ClassInfo (guarded by g_harvest_mtx); source for the event-file legend
// [DIAG] REP_CANONDIAG: for a sample of RAW rep covers, call the MAIN canonizer (cnvCheckNew via mode-1
// checkAndSave) and count how many raw covers are ALREADY canonical from cnvCheckNew's point of view.
std::function<bool(const unsigned char*)> g_checkCanonMode1;
std::atomic<long long> g_rawCanonTot{ 0 }, g_rawCanonPass{ 0 };
bool g_canonDiag = false;
...
struct RepWorker {
    ...
    // Serialize the cover in ITS CURRENT labeling (identity relabel), factors sorted -- the same
    // scheme canonKey uses per frame, with p = identity. The cover is CANONICAL iff this equals its
    // canonKey (best), i.e. the cover's own labeling already achieves the lex-min. Used to count each
    // class exactly ONCE (at the block holding its canonical labeling) => an intrinsic, run-order-
    // independent new-class density that INTEGRATES to the true class count (no recapture needed).
    std::string rawSerialize(const std::vector<Match>& f) {
        std::vector<Match> rel(f.begin(), f.end());
        std::sort(rel.begin(), rel.end());
        std::string s; s.reserve(NM * N);
    ...
    void emit() {
        ...
        }
        {
            ...
            // event list (REP_EVENTFILE): record EVERY cover as "<classIdx> <blockIdx>". classIdx is a
            // compact 1-based id assigned at first detection (= g_harvest.size() at that moment). The full
            // (class, block) event stream is the sufficient statistic: every ordinal/occurrence/first-seen
            // curve and the per-class detection histogram derive from it in post-processing.
            std::string raw = rawSerialize(chosen);      // this cover's own-labeling serialization
            std::string rev(raw.rbegin(), raw.rend());   // reversed, for reverse-lex ranking (keyed on the back)
            size_t hsh = std::hash<std::string>()(raw);  // position-uncorrelated key
            long long curBlk = g_curBlock.load(std::memory_order_relaxed);
            if (cdet == 1) g_classInfo[key] = { (int)g_harvest.size(), lastAut, raw, curBlk, rev, curBlk, hsh, curBlk };
            else { ClassInfo& ci = g_classInfo[key];
                if (raw < ci.minRaw) { ci.minRaw = raw; ci.canonBlk = curBlk; }              // lex-min block
                if (rev < ci.minRev) { ci.minRev = rev; ci.canonBlkRev = curBlk; }           // reverse-lex-min block
                if (hsh < ci.minHash) { ci.minHash = hsh; ci.canonBlkHash = curBlk; }        // hash-min block
            }
            if (g_eventFile) { fprintf(g_eventFile, "%d %lld\n", g_classInfo[key].idx, curBlk); fflush(g_eventFile); }   // "<classIdx> <blockIdx>"; flushed per cover (per-class canonical blocks are in the legend)
            // [DIAG] REP_CANONDIAG: is THIS raw cover already canonical per the main cnvCheckNew? (sampled, serialized here)
            if (g_canonDiag && g_checkCanonMode1 && g_rawCanonTot.load() < 20000) {
                unsigned char rawSrc[NM * N];
                for (int k = 0; k < NM; k++) adj_to_src(chosen[k].data(), rawSrc + k * N);
                bool c = g_checkCanonMode1(rawSrc);
                g_rawCanonTot.fetch_add(1, std::memory_order_relaxed);
                if (c) g_rawCanonPass.fetch_add(1, std::memory_order_relaxed);
            }
            // occurrence-ordinal buckets: this cover is EXACTLY the cdet-th detection of its class
            if      (cdet == 1)  g_blkN1.fetch_add(1, std::memory_order_relaxed);
            else if (cdet == 2)  g_blkN2.fetch_add(1, std::memory_order_relaxed);
            else if (cdet == 5)  g_blkN5.fetch_add(1, std::memory_order_relaxed);
            else if (cdet == 10) g_blkN10.fetch_add(1, std::memory_order_relaxed);
            if (cdet == 1) {
                ...
                if (g_printRawLeft > 0) {   // [DIAG] REP_PRINTRAW: the alpha-frame cover as the
                    ...
                    printf("\n[K16-REP] RAW search-frame cover, class #%zu (v4Row = partner of 0, v4 = partner of 1):\n", g_harvest.size());
                    for (const auto& F : fs) {
    ...
    void splitNode(bool parentTrivial, std::vector<std::vector<Match>>& outChildren, std::vector<char>& outTrivial) {
        ...
        std::vector<int> repIdx;
        bool childTrivial;
        if (parentTrivial) {                              // trivial up-path -> skip setwiseStab, explore all, stay trivial
...
void K16A2::runRepresentativeMethod(int order, int target) {
    initEdges();
    long long symCap = K16_REP_SYMCAP;                                    // overridable at runtime via env REP_SYMCAP
    if (const char* env = std::getenv("REP_SYMCAP")) symCap = atoll(env);   // e.g. force 2^7.1^2 over-cap for the block sweep
    ...
    g_target = target; g_stop.store(false); g_harvest.clear(); g_classInfo.clear(); g_curBlock.store(-1); g_banked.store(0); g_f1.store(0); g_f2.store(0);   // harvest mode (target>0 -> stop after `target` classes)
    ...
    if (m_bPrint) {
        if (noSkip) printf("[K16-REP] REP_NOSKIP: odd-prime parity-theorem skip DISABLED (theorem-verification mode)\n");
        if (estProbes > 0) printf("[K16-REP] REP_ESTIMATE=%d : Knuth tree-size probes per rep -- NO SEARCH, output is an ESTIMATE, not a result\n", estProbes);

        if (target > 0) printf("[K16-REP] HARVEST MODE: stop after %d distinct classes (COMPLETE only if %d == true count, else a LOWER BOUND)\n", target, target);
        if (rawShard) printf("[K16-REP] REP_RAWSHARD: branches = raw potential candidates (symmetry NOT reduced) -- LEVEL counts are a superset, mid-range seek is cheap; distinct-set correct via global dedup\n");
        {
            int nf = 0; for (int i = 0; i < 4; i++) if (m_v4Row[i] && m_v4Val[i]) nf++;
            if (nf) {
                printf("[K16-REP] V4 SHARD FILTERS (search frame):");
                ...
                for (int i = 0; i < 4; i++) {
                    ...
                    if (!r || !v || r < 2 || r > N - 1 || v < 2 || v > N - 1)
                        printf("[K16-REP] *** V4 FILTER %d INVALID: row=%d val=%d (both must be set, in 2..%d) -- shard is EMPTY\n", i + 1, r, v, N - 1);
                    else if (r == v)
                        printf("[K16-REP] *** V4 FILTER %d INVALID: row == val == %d (no vertex partners both 0 and 1) -- shard is EMPTY\n", i + 1, r);
                    for (int j = i + 1; j < 4; j++) {
                        if (!m_v4Row[j] || !m_v4Val[j]) continue;
                        if (m_v4Row[j] == r && m_v4Val[j] != v)
                            printf("[K16-REP] *** V4 FILTERS %d,%d CONTRADICT: row %d demands val %d AND %d -- shard is EMPTY\n", i + 1, j + 1, r, v, m_v4Val[j]);
                        if (m_v4Val[j] == v && m_v4Row[j] != r)
                            printf("[K16-REP] *** V4 FILTERS %d,%d CONTRADICT: val %d demanded in rows %d AND %d (edge {1,%d} is in ONE factor) -- shard is EMPTY\n", i + 1, j + 1, v, r, m_v4Row[j], v);
                    }
                }
            }
        }
        if (g_printRawLeft > 0) printf("[K16-REP] REP_PRINTRAW=%d : raw search-frame cover of the first %d classes will be printed\n", g_printRawLeft, g_printRawLeft);
        fflush(stdout);
    }
    ...
    for (int typeIdx = 0; typeIdx < g_numTypes; typeIdx++) {
        ...
        if (isE9 || isS3) {
            ...
            if (sh.gElems.size() != (isE9 ? 8u : 5u)) {   // paranoia: generator closure must be the full group
                if (m_bPrint) { printf("[K16-REP]  type %-14s (type %d/%d) SKIPPED: INTERNAL closure size %zu != %u -- report this\n", tstr, tIdx, g_numTypes, sh.gElems.size(), isE9 ? 8u : 5u); fflush(stdout); }
                continue;
            }
        ...
        if ((isE9 || isS3) && !sh.Calpha.empty()) {   // paranoia: every symmetry element must commute with the whole group
            ...
            if (!okC) { if (m_bPrint) { printf("[K16-REP]  type %-14s SKIPPED: INTERNAL centralizer element does not commute -- report this\n", tstr); fflush(stdout); } continue; }
        }
        ...
        if (!sh.Calpha.empty()) {
            ...
            if (rangeMode || shardLevel > 1) {
                if (isE9 || isS3) {
                    ...
                    if (!okG) { if (m_bPrint) { printf("[K16-REP]  type %-14s SKIPPED: INTERNAL centralizer generator does not commute -- report this\n", tstr); fflush(stdout); } continue; }
                } else
            ...
            if (isV4) {   // cannot happen (V4 |C| << cap); the CGT fallback below is cyclic-only
                if (m_bPrint) { printf("[K16-REP]  type %-14s SKIPPED: V4 centralizer over cap (unsupported path)\n", tstr); fflush(stdout); }
                continue;
            }
        ...
        if (estProbes > 0) {
            if (sh.Calpha.empty()) {
                if (m_bPrint) { printf("[K16-REP]  type %-14s ESTIMATE SKIPPED: over-cap type (estimator supports the enumerable path only)\n", tstr); fflush(stdout); }
                continue;
            }
            if (reps.empty()) {   // root-collapsed type: exhausted during collection -> 0 nodes, 0 classes
                if (m_bPrint) { printf("[K16-REP]  type %-14s ESTIMATE: reps 0 -> est cover-nodes TOTAL ~ 0 (type exhausts at the root)\n", tstr); fflush(stdout); }
                continue;
            }
            ...
            if (m_bPrint) {
                printf("[K16-REP]  type %-14s ESTIMATE: reps %zu, probes/rep %d -> est cover-nodes TOTAL ~ %.3g (largest rep #%zu ~ %.3g mean, %.3g max-probe)\n",
                       tstr, reps.size(), estProbes, total, worstR, worstMean, estMax[worstR]);
                printf("[K16-REP]  type %-14s ESTIMATE: ETA = above TOTAL / (nodes-per-second of a real run on the same machine/threads)\n", tstr);
                ...
                printf("[K16-REP]  type %-14s ESTIMATE per block: idx  est-nodes  xMean  max-probe\n", tstr);
                for (size_t r = 0; r < reps.size(); r++)
                    printf("[K16-REP]    block %3zu  %10.3g  %6.2fx  %10.3g\n",
        ...
        if (sh.Calpha.empty() && diveMode && shardLevel == 0) {   // pure fast search (no sweep); with REP_LEVEL the sweep's runBlock dives per block
            ...
            if (reps.empty()) { if (m_bPrint) { printf("[K16-REP]  type %-14s FAST SEARCH: no root reps -> 0\n", tstr); fflush(stdout); } continue; }
            if (m_bPrint) { printf("[K16-REP]  type %-14s FAST SEARCH (backtracking dives, budget %lld nodes/restart) x %d threads -> run until target/kill...\n", tstr, diveBudget, nThreads); fflush(stdout); }
            ...
            if (rangeMode && chunkStep > 0) {
                  ...
                  [&](const std::vector<Match>& cover, bool triv, int depth) {
                    ...
                    if (depth == shardLevel) {                 // a level-L branch (index = visited)
                        ...
                        if (!chunkHalt.load(std::memory_order_relaxed)) {   // each systematic block runs to its END (no mid-block cut)
                            g_blockCovers.store(0); g_blkN1.store(0); g_blkN2.store(0); g_blkN5.store(0); g_blkN10.store(0);   // per-block estimate counters
                            g_curBlock.store(i, std::memory_order_relaxed);   // event-list: tag every cover found in this block with block index i
                            runBlock(cover, triv ? 1 : 0);
                            chunkBlocks++;
                            if (g_estFile && !diveMode)   // FULLY-scanned block: "<blockIdx> <rawCovers> <n1> <n2> <n5> <n10>" (nK = covers that were the exactly-Kth detection of their class; n1 = newDistinct)
                                { fprintf(g_estFile, "%lld %lld %lld %lld %lld %lld\n", i, g_blockCovers.load(), g_blkN1.load(), g_blkN2.load(), g_blkN5.load(), g_blkN10.load()); fflush(g_estFile); }
                ...
                if (!diveMode && std::getenv("REP_ESTFILE")) {   // per-block estimate log (fresh per run; step in the name)
                    ...
                    if (m_bPrint) { printf("[K16-REP]  type %-14s ESTIMATE LOG -> %s (%s)\n", tstr, epath, g_estFile ? "fresh" : "OPEN FAILED"); fflush(stdout); }
                }
                if (!diveMode && std::getenv("REP_EVENTFILE")) {   // full (class,block) event list (fresh per run; step in the name)
                    char vpath[256]; snprintf(vpath, sizeof(vpath), "k%d_%d_step%lld_events.txt", N, tIdx, chunkStep);
                    g_eventFile = fopen(vpath, "w");
                    if (g_eventFile) fprintf(g_eventFile, "# k%d t%d step%lld  <classIdx> <blockIdx>  (one line per COVER, incl. re-finds; per-class canonical blocks in the EOF legend)\n", N, tIdx, chunkStep);
                    if (m_bPrint) { printf("[K16-REP]  type %-14s EVENT LIST -> %s (%s)\n", tstr, vpath, g_eventFile ? "fresh" : "OPEN FAILED"); fflush(stdout); }
                }
                int sweepPass = 0;
                do {
                    if (m_bPrint && ++sweepPass > 1) { printf("[K16-REP]  type %-14s SWEEP PASS %d (cum %zu distinct) -> re-dive the range...\n", tstr, sweepPass, g_harvest.size()); fflush(stdout); }
                ...
                if (g_eventFile) {   // append the legend: classIdx -> 2*|Aut| + a stable 64-bit hash of canonKey (matches classes across different-step runs)
                    fprintf(g_eventFile, "# legend: <classIdx> <2*|Aut|> <hash64hex> <detections> <blkLexMin> <blkRevLex> <blkHash>  (block of the class's canonical cover under 3 rules: front lex-min, reverse-lex-min, hash-min)\n");
                    std::vector<std::pair<int, std::string>> byIdx;
                    for (const auto& kv : g_classInfo) byIdx.push_back({ kv.second.idx, kv.first });
                    std::sort(byIdx.begin(), byIdx.end());
                    for (const auto& p : byIdx) {
                        unsigned long long h = (unsigned long long)std::hash<std::string>()(p.second);
                        const ClassInfo& ci = g_classInfo[p.second];
                        fprintf(g_eventFile, "# %d %d %016llx %lld %lld %lld %lld\n", p.first, ci.aut2, h, g_harvest[p.second], ci.canonBlk, ci.canonBlkRev, ci.canonBlkHash);
                    }
                    fclose(g_eventFile); g_eventFile = nullptr;
                }
            ...
            if (rangeMode && deepShard && shardHi > 0) {
                ...
                g_reps_total.store((long long)queue.size());
                if (m_bPrint) { printf("[K16-REP]  type %-14s BLOCK THREADING: %zu block(s) seeded -> shared work queue, all %d threads\n", tstr, queue.size(), nThreads); fflush(stdout); }
                if (sampling && m_bPrint) { printf("[K16-REP]  type %-14s SAMPLE: every %lld-th branch of [%zu, %zu) = %zu branches, cap %lld nodes each (0 = none)\n", tstr, sampleStride, rangeStart, rangeStart + shardHi, sampleIdx.size(), sampleCap); fflush(stdout); }
        ...
        if (sampleStride > 0 && !sampleIdx.empty() && m_bPrint) {   // SAMPLE summary: the stride-scaled estimate of the slice
            ...
            printf("[K16-REP]  type %-14s SAMPLE summary: %zu branches, %lld finished, %lld capped; nodes finished %lld (mean %.0f, max %lld), all %lld\n",
                   tstr, sampleIdx.size(), fin, capped, sumFin, fin ? (double)sumFin / (double)fin : 0.0, mx, sumAll);
            printf("[K16-REP]  type %-14s SAMPLE estimate of the slice: stride %lld x %lld = %.3e nodes%s\n",
        ...
        if (m_bPrint && diveMode && sh.Calpha.empty()) {
            printf("[K16-REP]  type %-14s FAST SEARCH: %lld random restarts, %zu new distinct classes (cum %zu)\n",
    ...
    if (g_canonDiag) { printf("[K16-REP]  CANONDIAG: raw rep covers that are ALREADY canonical per cnvCheckNew = %lld / %lld (%.3f%%)\n",
        g_rawCanonPass.load(), g_rawCanonTot.load(), g_rawCanonTot.load() ? 100.0 * g_rawCanonPass.load() / g_rawCanonTot.load() : 0.0); fflush(stdout); }
    g_checkCanonMode1 = nullptr;
    ...
    if (m_bPrint) {
        ...
        if (harvested)   // "stopped at a target" is not the same outcome as "finished", so it still speaks up
            printf("[K16-REP] HARVESTED %zu classes (target %d reached -- NOT proven complete)\n", gcanon.size(), g_target);
        fflush(stdout);
    }
}
