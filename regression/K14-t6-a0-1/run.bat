@echo off
REM ============================================================================
REM  regression\K14-t6-a0-1 -- guards the search engine used by K18-t8-a0.
REM  K14 type t6 = 2^6 1^2 (two fixed points), branch a=0, one block column through the same block
REM  driver K18 uses.  EXPECTED: 2 record(s), compared against
...
SET "RC=%ERRORLEVEL%"
REM  The classes are the coarse signal; the node count is the sensitive one.  A change that
REM  leaves the class set alone but walks a different tree is what this second check catches.
IF NOT "%RC%"=="0" GOTO :VERDICT
"%PERL%" "..\check_nodes.pl" "%CASE%" "%LOG%" 44047
